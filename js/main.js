...
var players = [
  {
    ...
    match: 0,
    attempts: 0,
    pokemonShuffle: $.extend(true, [], pokemons)
  },
  {
    pokemon: null,
    name: "",
...
function modalInput() {
  ...
  switch (currentState) {
    ...
    case "getPlayer1Pokemon":
      ...
      setTimeout(function() {
        players[currentPlayer].pokemon = parseInt($(_this).attr("data-target"));
...
function populateGameData(playerID, currentPlayer, playerPokemon) {
  ...
  if ($(window).width() >= 700) {
    $(playerPokemon).append(
      $("<div>", {
        class:
          "centerPokemonImg " +
          selectedPokemon[players[currentPlayer].pokemon].class
      })
    );
  } else {
    $(playerPokemon).append(
...
function populateBoard(playerID) {
  ...
  for (var x = 0; x < playerID.length; x++) {
    ...
    var frontCardContent = $("<div>", {
      class: "frontCardImg " + playerID[x].image
    });
...
function firstCard(location) {
  ...
  first_card_clicked =
    players[currentPlayer].pokemonShuffle[indexClick.firstClick];
  if (
    players[currentPlayer].pokemonShuffle[indexClick.firstClick].flipped == true
  ) {
...
function secondCard(location) {
  ...
  second_card_clicked =
    players[currentPlayer].pokemonShuffle[indexClick.secondClick];

  if (
    players[currentPlayer].pokemonShuffle[indexClick.secondClick].flipped ==
    true
  ) {
...
function accuracy() {
  players[currentPlayer].accuracy = Math.floor(
    (players[currentPlayer].match / players[currentPlayer].attempts) * 100
  );
...
function playerTurnScreen() {
  ...
  var modalPlayerText = $("<div>", {
    class: "modalPlayerText",
    text: players[currentPlayer].name + "'s Turn"
  });
  var modalContainer = $("<div>", { class: "modalPlayerTurn" });
  var modalClass =
    "modal" +
    selectedPokemon[players[currentPlayer].pokemon].class.substring(6);
  var innerModalContainer = $("<div>", {
    class: "modalPlayerPokemonBox " + modalClass
  });
...
function winner() {
  ...
  var modalPlayerText = $(".modalPlayerText").text(
    players[currentPlayer].name + " Wins"
  );
  var playButton = $("<button>", {
    class: "playerWon btn btn-outline-primary btn-lg",
    text: "Play Again"
  });
...
function sneakPeak(e) {
  if (
    ...
    players[currentPlayer].playersPeaks[players[currentPlayer].peakCount] =
      e.target;
    for (var x = 0; x < $(".gameArea>.pickCard").length; x++) {
      flip(
        $(".gameArea>.pickCard")[x],
        players[currentPlayer].pokemonShuffle[x]
      );
    }
    setTimeout(function() {
      for (var _x = 0; _x < $(".gameArea>.pickCard").length; _x++) {
        unFlipPeak(
          $(".gameArea>.pickCard")[_x],
          players[currentPlayer].pokemonShuffle[_x].flipped
        );
      }
    }, 3000);
...
function playAgain() {
  ...
  switch (currentPlayer) {
    case 0:
      ...
      for (
        var change1 = 0;
        change1 < players[currentPlayer].playersPeaks.length;
        change1++
      ) {
    ...
    case 1:
      for (
        var change2 = 0;
        change2 < players[currentPlayer].playersPeaks.length;
        change2++
      ) {
